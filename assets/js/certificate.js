...
async function resolveData() {
    ...
    try {
        if (enrollmentParam) {
            ...
            if (Array.isArray(enrolls)) {
                const foundEnroll = enrolls.find(
                    (e) => String(e.id) === String(enrollmentParam),
                );
                if (foundEnroll) {
    ...
    try {
        ...
        if (Array.isArray(courses) && (courseParam || courseTitle)) {
            ...
            const found = courses.find(
                (c) =>
                    String(c.id) === String(needle) ||
                    String(c.title) === String(needle),
            );
    ...
    try {
        ...
        if (Array.isArray(students) && needle) {
            const found = students.find(
                (s) =>
                    String(s.nationalId) === String(needle) ||
                    String(s.id) === String(needle) ||
                    (s.email && s.email === needle),
            );
